...
if 'total_questions' not in st.session_state:
    st.session_state.total_questions = 1  # 처음 시작할 때 1로 설정
...
if st.button("정답 확인"):
    ...
    if user_answer.lower() == correct_word.lower():
        st.success("정답입니다!")
        st.session_state.correct_answers += 1  # 여기서 맞춘 문제 수를 증가시킵니다.
        st.write(f"맞춘 문제 수: {st.session_state.correct_answers}")  # 업데이트된 맞춘 문제 수를 표시합니다.
    else:
    ...
    # 다음 문제 버튼 추가
    if st.button("다음 문제"):
        st.session_state.current_question = None  # 다음 문제를 위해 current_question을 초기화합니다.
        st.session_state.total_questions += 1  # 다음 문제로 넘어갈 때 총 문제 수 증가
        st.rerun()

# 페이지 상단에 총 문제 수와 맞춘 문제 수 표시
